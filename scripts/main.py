...
if __name__ == "__main__":
    ...
    dataset = ds.get_train_data() #2
    valid_dataset = ds.get_valid_data() #3
    test_dataset = ds.get_test_data() #4
    dataset = dataset.repeat(NUM_EPOCHS) #5
    # dataset = dataset.shuffle(100)
    dataset = dataset.batch(BATCH_SIZE) #6
    dataset = dataset.prefetch(1) #7
    data_iter = dataset #8
    # valid_dataset = valid_dataset.repeat(NUM_EPOCHS)
    # dataset = dataset.shuffle(100)
    valid_dataset = valid_dataset.batch(BATCH_SIZE) #9
    valid_dataset = valid_dataset.prefetch(1)  #10
    # test_dataset = test_dataset.repeat(NUM_EPOCHS)
    # dataset = dataset.shuffle(100)
    test_dataset = test_dataset.batch(1) #11
    test_dataset = test_dataset.prefetch(1) #12
