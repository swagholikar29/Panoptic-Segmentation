####mask_matching in tensorflow 2.0#####
...
class MaskMatching(nn.Module):
    ...
    def forward(self, gt_segs, gt_masks, keep_inds=None):
        ...
        matched_gt = tf.ones_like(gt_segs) * -1 #creates a tensor of size gt_segs with all values 1 
        matched_gt = tf.where(gt_segs <= config.dataset.num_seg_classes - config.dataset.num_classes, gt_segs, matched_gt) #takes the value from gt_segs or matched_gt as per condition
        matched_gt = tf.where(gt_segs >= 255, gt_segs, matched_gt)
...
class PanopticGTGenerate(nn.Module):
    ...
    def forward(self, rois, bbox_pred, cls_score, label, gt_rois, cls_idx, seg_gt, mask_gt, im_shape):
        ...
        rois = tf.make_ndarray(rois) #rois.data.cpu().numpy() 
        bbox_pred = tf.make_ndarray(bbox_pred) #bbox_pred.data.cpu().numpy()
        cls_score = tf.make_ndarray(cls_score) #cls_score.data.cpu().numpy()
        cls_pred = tf.math.argmax(cls_score, axis=1) #np.argmax(cls_score, axis=1)
        label = tf.make_ndarray(label) #label.data.cpu().numpy()
        gt_rois = tf.make_ndarray(gt_rois) #gt_rois.cpu().numpy()

        rois = rois[:, 1:]

        bbox_overlap = bbox_overlaps(rois, gt_rois[:, 1:])  # #rois x #gt_rois
        max_bbox_overlap = tf.math.argmax(bbox_overlap, axis=1) #np.argmax(bbox_overlap, axis=1)
        max_overlap = np.ones((gt_rois.shape[0]), dtype=np.int32) * -1

        matched_gt = tf.ones_like(seg_gt) * -1
        matched_gt = tf.where(seg_gt <= config.dataset.num_seg_classes - config.dataset.num_classes, seg_gt, matched_gt)
        matched_gt = tf.where(seg_gt >= 255, seg_gt, matched_gt)
        ...
        return tf.convert_to_tensor(rois).device(matched_gt), tf.convert_to_tensor(label).device(matched_gt), matched_gt
        ###doubtful about return conversion, whether to use this or tf.convert_to_tensor###
