...
class RangeNetModel(Model): 
    def __init__(self, inp_shape=(None, 64, 1024, 5), rnn_flag=False, pixel_shuffle=False):
        ...
        self.encoder = Encoder(pixel_shuffle=pixel_shuffle) #1
        self.decoder = Decoder(pixel_shuffle=pixel_shuffle) #2
        self.semantic_head = SegmentationHead() #3
        self.softmax = Softmax(axis=3) #4
