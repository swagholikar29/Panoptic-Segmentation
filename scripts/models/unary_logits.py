...
import math
from mrcnn.config.config import config


class MaskTerm(tf.Module):
    ...
    def forward(self, masks, boxes, cls_indices, seg_score):
        ...
        mask_energy = tf.zeros((seg_score.shape[0], masks.shape[0], seg_score.shape[2], seg_score.shape[3]), tf.int32)
        ...
        for i in range(cls_indices.shape[0]):
            ...
            w = max(w, 1)
            h = max(h, 1)
            'mask = F.upsample(masks[i, 0, :, :].reshape(1, 1, config.network.mask_size, config.network.mask_size), size=(h, w), mode=''bilinear'', align_corners=False)
            mask = tf.keras.layers.UpSampling2D(size=(h, w), data_format=None, interpolation='bilinear') (masks[i, 0, :, :].reshape(1, 1, config.network.mask_size, config.network.mask_size))
...
class SegTerm(tf.Module):
    ...
    def forward(self, cls_indices, seg_score, boxes):
        ...
        if cls_indices.size == 0:
            return seg_energy, tf.ones_like(seg_energy[[0], [0], :, :]).reshape(1, 1, seg_energy.shape[2], seg_energy.shape[3]) * -10
        else:
            seg_inst_energy = tf.zeros((seg_score.shape[0], cls_indices.shape[0], seg_score.shape[2], seg_score.shape[3]), tf.int32)
            for i in range(cls_indices.shape[0]):
                if cls_indices[i] == 0:
                    continue
                y0 = int(boxes[i][1])
