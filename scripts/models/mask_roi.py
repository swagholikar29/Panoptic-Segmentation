...
#from torch.nn.modules.module import Module (what to import for this)
#from torch.autograd import Variable (what to import for this)
...
class MaskROI(Module):
    def __init__(self, clip_boxes, bbox_class_agnostic, top_n, num_classes, nms_thresh=None, class_agnostic=False, score_thresh=None):
        super(MaskROI, self).__init__() ##(check if need to change)
    ...
    def forward(self, bottom_rois, bbox_delta, cls_prob, im_info, nms=True, cls_score=None, cls_label=None):
        ...
        context = tf.device(bottom_rois) #check if correct
        nms = gpu_nms_wrapper(self.nms_thresh, device_id=context)


        #bottom_rois     = bottom_rois.cpu().detach().numpy()
        bottom_rois     = tf.make_ndarray(bottom_rois)
        #bbox_delta      = bbox_delta.cpu().detach().numpy()
        bbox_delta      = tf.make_ndarray(bbox_delta)
        cls_prob        = cls_prob.cpu() ##(check how to change)
        cls_prob_np = cls_prob.detach().numpy() ##(check how to change)
        if cls_score is not None:
            cls_prob = cls_score.cpu() ##(check how to change)
        if cls_label is not None:
            cls_label = cls_label.cpu() ##(check how to change)
        ...
        if self.class_agnostic:
            ...
            cls_prob = cls_prob[:, 1:].contiguous().view(-1, 1)  ##(check how to change)
            # n x 1 -> n x 2
            #cls_prob = torch.cat([torch.zeros_like(cls_prob), cls_prob], dim=1)
            cls_prob = tf.concat([tf.zeros_like(cls_prob), cls_prob], dim=1)
            ...
            if cls_label  is not None:
                cls_label = cls_label.view(-1, 1).expand(-1, self.num_classes).contiguous().view(-1, 1)  ##(check how to change)
        ...
        for j in range(1, self.nms_classes):
            ...
            scores_th[j] = cls_prob[tf.cast(tf.convert_to_tensor(inds),long()), j][tf.cast(tf.convert_to_tensor(np.array(keep)),long())] ##check if correct
        ...
        # Limit to max_per_image detections **over all classes** ##(check what to import)
        if config.test.max_det > 0:
            ...
            if len(image_scores) > config.test.max_det:
                ...
                for j in range(1, self.nms_classes):
                    ...
                    scores_th[j] = scores_th[j][tf.convert_to_tensor(np.array(keep))]
                    cls_idx[j] = cls_idx[j][keep]
                    if cls_score is not None:
                        feat_th[j] = feat_th[j][tf.convert_to_tensor(np.array(keep))]
                    if cls_label is not None:
        ...
        if cls_label is not None:
            ...
            scores_th = tf.concat(scores_th[1:]).to(context, non_blocking=True)
            return scores_th, \
                   ...
                   cls_label ##(check how to convert)
        ...
        if scores.size == 0:
            ...
            return torch.from_numpy(scores).float().to(context, non_blocking=True), \
                   ...
                   torch.from_numpy(cls_idx).long().to(context, non_blocking=True) ##(check how to convert)
        else:
            scores_th = torch.cat(scores_th[1:]).to(context, non_blocking=True)
            return scores_th, \
                   torch.from_numpy(boxes).float().pin_memory().to(context, non_blocking=True), \
                   torch.from_numpy(cls_idx).long().pin_memory().to(context, non_blocking=True) ##(check how to convert)
    ...
    def bbox_transform(bbox, bbox_delta, im_info=None):
        ...
        dx = dx / config.network.bbox_reg_weights[0]  ##check file structure
        dy = dy / config.network.bbox_reg_weights[1]  ##check file structure
        dw = dw / config.network.bbox_reg_weights[2]  ##check file structure
        dh = dh / config.network.bbox_reg_weights[3]  ##check file structure

        transformed_center_x = center_x.view(-1, 1) + bbox_width.view(-1, 1) * dx ##check how to change
        transformed_center_y = center_y.view(-1, 1) + bbox_height.view(-1, 1) * dy ##check how to change
        transformed_width = bbox_width.view(-1, 1) * dw.exp() ##check how to change
        transformed_height = bbox_height.view(-1, 1) * dh.exp() ##check how to change
        ...
        # transformed_bbox = torch.cat([transformed_xmin, transformed_ymin, transformed_xmax, transformed_ymax], dim=1) ##was already commented
        #transformed_bbox = torch.zeros_like(bbox_delta)
        transformed_bbox = tf.zeros_like(bbox_delta)
        ...
        transformed_bbox = tf.reshape((transformed_bbox, transformed_bbox.shape[0], -1, 4)) ##changed


        if im_info is not None:
            # [1, 2]
            #im_wh = torch.from_numpy(im_info[0, [1, 0]] - 1.0).to(transformed_bbox.device, non_blocking=True)
            im_wh = torch.from_numpy(im_info[0, [1, 0]] - 1.0).to(transformed_bbox.device, non_blocking=True) ##(check how to convert)
            # [1, 1, 4]
            im_wh = im_wh.repeat(1, 2).unsqueeze(0)
            transformed_bbox = tf.math.reduce_min(transformed_bbox, im_wh) ##changed
            transformed_bbox = tf.math.reduce_max(transformed_bbox, tf.zeros_like(transformed_bbox)) ##changed
